...
COLORREF color[7]={
	REB(255,32,83),RGB(252,222,250),RGB(255,0,0),RGB(255,0,0),RGB(255,2,2),RGB(255,0,8),RGB(255,5,5)
};
...
double screen_x(double x)
{
 ...
 {
 	int index=0;
 	double x1=0,y1=0,x2=0,y2=o;
 	for (double radian=0.1;radian<=2*PI;radian+=0.005)
 	{
 		x2=16*pow(sin(radian),3);
 		y2=13*cos(radian)-5*cos(2*radian)
 		-2*cos(3*radian)-cos(4*radian);
	 ...
	 for(double size=0.1,lightness=1.5;size<=20;size+=0.1)
	 {
	 	...
	 	for(int i=0:i<quantity;++i)
	 	{
	 		...
	 		{
	 			points[index].color=color=colors[creat_random(0,6)];
 ...
 int main()
 {
 	...
 	for(int frame=0;!_kbhit();)
 	{
 		...
 		frame==0?(shrink=false,extend=ture):--frame;
	 }
	 EndBatchDraw();
	 closegraph();
	 return 0;
